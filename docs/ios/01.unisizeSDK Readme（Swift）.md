...
- 26/09/15 v3.0.1 リリースに伴い「バージョン履歴」を更新。
- 26/08/21 v3.0（aunn コーディネート機能）への言及と関連ドキュメントへのリンクを追加。「バージョン履歴」を更新。
- 25/02/26 「バージョン情報」を更新
- 25/12/08 「バージョン情報」を更新
...
v3.0 より、aunn コーディネートにも対応しました。詳しくは付属のドキュメント「[11.aunn コーディネート機能の実装について（iOS）](./11.aunn%20コーディネート機能の実装について（iOS）.md)」をご覧ください。  
...
### v3.0
- 【iOS】aunn コーディネートに対応しました。`AunnCoordinate` / `AunnCoordinateTracking` / `AunnCoordinateMeasurement` クラスを追加しています。コーデ一覧の UI はホストアプリがネイティブ描画し、SDK はコーデの取得・行動計測・体型登録画面の表示を提供します。詳しくは付属のドキュメント「[11.aunn コーディネート機能の実装について（iOS）](./11.aunn%20コーディネート機能の実装について（iOS）.md)」をご覧ください。
...
aunn コーディネートは、ユーザーの体型・興味に合わせてスタッフコーディネート（コーデ）をレコメンドする機能です。  
商品詳細画面などにスタッフコーデの一覧を表示し、ユーザーが体型を登録すると、その体型に合うスタッフのコーデが優先的に並ぶようになります。  
  
コーデ一覧の UI はホストアプリがネイティブ描画し、SDK はコーデデータの取得・行動計測・体型登録画面（`WKWebView`）の表示を担います。  
実装方法などは、付属のドキュメント「[11.aunn コーディネート機能の実装について（iOS）](./11.aunn%20コーディネート機能の実装について（iOS）.md)」をご覧ください。
