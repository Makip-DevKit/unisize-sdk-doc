...
- 【Android】aunn コーディネート機能（スタッフコーデ）に対応しました。`AunnCoordinate` / `AunnCoordinateTracking` / `AunnCoordinateMeasurement` クラスを追加しています。コーデ一覧の UI はホストアプリがネイティブ描画し、SDK はコーデの取得・行動計測・体型登録画面の表示を提供します。
- 【iOS】iOS 版も同等のクラス（`AunnCoordinate` / `AunnCoordinateTracking` / `AunnCoordinateMeasurement`）で aunn コーディネート機能に対応しました。詳しくは「[11.aunn コーディネート機能の実装について（iOS）](./11.aunn%20コーディネート機能の実装について（iOS）.md)」をご覧ください。
- 【iOS】unisize バナーと aunn を同一アプリ内で併用する場合に、beid を自動で共有するようにしました。aunn で体型登録した結果は unisize バナーへ自動で反映されます。逆方向（unisize 側での体型登録を aunn コーデへ反映する）の配線と、unisize バナーを設置しない画面での `unisizeBeidWaitMs` の指定については「[11.aunn コーディネート機能の実装について（iOS）](./11.aunn%20コーディネート機能の実装について（iOS）.md)」をご覧ください。
- 【iOS】aunn コーデ一覧の表示時間計測に対応しました。`AunnCoordinate.startCoordinateViewTimeTracking(view:recType:)` で、コーデ一覧が画面内に 50% 以上・3 秒間表示されたときに計測を送信します。体型登録アプリの表示中は `pauseCoordinateViewTimeTracking()` / `resumeCoordinateViewTimeTracking()` で一時停止・再開できます。
- 【iOS】aunn 用のエラーコード（500 番台）を追加しました。詳細は「[04.エラーコード一覧（iOS）](./04.エラーコード一覧（iOS）.md)」をご覧ください。
- 【iOS】aunn コーディネート機能は v3.0 時点では Swift 専用です。Objective-C で開発されたプロジェクトからはご利用いただけません。

以下は、aunn コーディネート機能への対応にあわせて行った unisize バナー側の変更です。

- 【iOS】aunn で体型登録が完了したときに、表示中の unisize バナーを SDK が自動で再読込するようにしました。バナーが「登録済み」の表示に切り替わります。ホストアプリからバナーの再読込を呼び出す必要はありません。
- 【iOS】`UnisizeBanner` Class の `close()` が、表示中のすべてのバナーの登録を破棄してしまう問題を修正しました。これまでは複数の画面でバナーを表示している場合に、1 つの画面を閉じると他の画面のバナーが以後更新されなくなっていました。`close()` は自身のバナーのみを終了するようになります（Android 版と同じ挙動に統一）。
- 【iOS】unisize バナーの読み込みが中断されたとき（`NSURLErrorCancelled`／-999）に、エラーとして通知しないようにしました。バナーの再読込など正常な処理でも発生していたためです。
- 【iOS】`UnisizeBannerWebview` Class をコードから直接生成できるようにしました（`init(frame:)` の公開）。ストーリーボードを使用しない構成のアプリでもバナーを配置できます。
  
## v2.0.6

- 【iOS】xcframeworkにdSYMファイル（デバッグ用シンボル情報）を含むように変更しました。
※ iOS版のみのアップデートとなります。
