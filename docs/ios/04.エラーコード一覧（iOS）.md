...
## ドキュメントのバージョン情報
* 26/08/31 aunn コーディネート機能のエラーコード（500番台）を追加
* 24/06/06 初版
...
エラーメッセージは `AunnCoordinateDelegate` / `AunnCoordinateMeasurementDelegate` の `didFail`（`func aunn...(_:didFail error: UnisizeError)`）で取得できます。
aunn の実装については [11.aunn コーディネート機能の実装について（iOS）](./11.aunn%20コーディネート機能の実装について（iOS）.md) を参照してください。

| エラーコード | エラーコード | メッセージ  | 詳細     | 
| ------------ | ---------------- | -------- | -------- |
| 500          | aunnCoordinateApiError | Error: Failed to fetch the aunn coordinate list. | aunn サーバーからのスタッフコーデ一覧の取得に失敗 | 
| 501          | aunnAttributeApiError | Error: Failed to fetch the user attribute from the UNISIZE API. | unisize サーバーからのユーザー属性の取得に失敗 | 
| 502          | aunnBeidStatusApiError | Error: Failed to fetch the beid status from the UNISIZE API. | unisize サーバーからの体型登録状態の取得に失敗 | 
| 503          | aunnTrackingApiError | Error: Failed to send the aunn tracking log. | トラッキングログの送信に失敗 | 
| 504          | aunnParamInsufficiencyError | Error: An error occurred because a required aunn parameter was not set. | 必須パラメーター未設定。<br>※ SDK に設定したパラメーター関連のエラー | 
| 510          | aunnMeasurementWebViewJavascriptError | Error: A Javascript error has occurred in the measurement app. | 体型登録アプリの WKWebView で Javascript エラーが発生 | 
| 511          | aunnMeasurementWebViewReceivedError | Error: An error occurred while loading the measurement app web resources. | 体型登録アプリの WKWebView ロード中にエラーが発生 | 
| 512          | aunnMeasurementWebViewSslError | Error: An SSL-related error has occurred while loading the measurement app. | 体型登録アプリの WKWebView で SSL エラーが発生 |

**※ 次の 2 つのエラーコードは `didFail` では通知されません。**

- **503（`aunnTrackingApiError`）**：トラッキングログの送信は fire-and-forget（primary の送信に失敗した場合は fallback 先へ再送）のため、最終的に失敗しても Delegate へは通知されません。エラーコードの定義のみ存在します。
- **510（`aunnMeasurementWebViewJavascriptError`）**：体型登録アプリ内の JavaScript エラーはそのままフローを継続できるため、エラーログの送信のみを行います。 
