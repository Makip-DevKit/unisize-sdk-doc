...
- 26/09/15 v3.0.1 リリースに伴い「バージョン履歴」を更新。「導入手順」>「パッケージで導入」の依存関係のバージョン表記を更新。
- 26/08/13 v3.0（aunn コーディネート）への言及と関連ドキュメントへのリンクを追加。「バージョン履歴」を更新。
...
- 【Android】aunn コーディネートに対応しました。`AunnCoordinate` / `AunnCoordinateTracking` / `AunnCoordinateMeasurement` クラスを追加しています。詳しくは付属のドキュメント「[11.aunn コーディネート機能の実装について（Android）](./11.aunn%20コーディネート機能の実装について（Android）.md)」をご覧ください。
...
```
dependencies {
    implementation("jp.co.makip:unisize-sdk:x.x.x")

    ・・・
}
```

- バージョンは公開されている最新版を指定してください。
...
コーディネート一覧の UI はホストアプリがネイティブ描画し、SDK はコーディネートデータの取得・行動計測・体型登録画面（WebView）の表示を担います。  
実装方法などは、付属のドキュメント「[11.aunn コーディネート機能の実装について（Android）](./11.aunn%20コーディネート機能の実装について（Android）.md)」をご覧ください。  
※ aunn コーディネートのサンプルコードは Kotlin で記載しています。Java プロジェクトからも同じ Class をご利用いただけます。
