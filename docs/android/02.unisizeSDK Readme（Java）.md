...
1. unisizeSDK ダウンロードサイトから unisizeSDK for Android の zip ファイルをダウンロード、解凍します。
...
```
dependencies {
    implementation("jp.co.makip:unisize-sdk:1.3.0")
...
- username、password は GitHub アカウントのユーザー名、トークン（Personal access tokens）を設定してください。（サンプルでは env から取得しています。）
