...
## v3.0.1
unisizeSDK v3.0 とは互換性がありますので、実装を変更することなく、そのままライブラリ（unisizeSDK-release.aar）を差し替えるだけでアップデート可能です。 
v2.0.6 以前からアップデートされる場合は、下記「v3.0」の内容もあわせてご確認ください。 
  
## v3.0
unisizeSDK v2.0.6 / v2.0.5 とは互換性がありますので、実装を変更することなく、そのままライブラリ（unisizeSDK-release.aar）を差し替えるだけでアップデート可能です。 
v3.0 で追加された aunn コーディネートは追加機能のため、ご利用にならない場合は既存の実装を変更する必要はありません。 
aunn コーディネートをご利用になる場合は、付属のドキュメント「[11.aunn コーディネート機能の実装について（Android）](./11.aunn%20コーディネート機能の実装について（Android）.md)」をご覧ください。 
  
## v2.0.5
unisizeSDK v2.0 とは互換性がありますので、実装を変更することなく、そのままライブラリ（unisizeSDK-release.aar）を差し替えるだけでアップデート可能です。 
