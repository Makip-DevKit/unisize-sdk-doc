...
- 【Android】aunn コーディネートに対応しました。`AunnCoordinate` / `AunnCoordinateTracking` / `AunnCoordinateMeasurement` クラスを追加しています。コーディネート一覧の UI はホストアプリがネイティブ描画し、SDK はコーディネートの取得・行動計測・体型登録画面（WebView）の表示を提供します。詳しくは「[11.aunn コーディネート機能の実装について（Android）](./11.aunn%20コーディネート機能の実装について（Android）.md)」をご覧ください。
- 【Android】unisize バナーと aunn を同一アプリ内で併用する場合に、beid を自動で共有するようにしました。aunn で体型登録した結果は unisize バナーへ自動で反映されます。逆方向（unisize 側での体型登録を aunn コーディネートへ反映する）の配線と、unisize バナーを設置しない画面での `unisizeBeidWaitMs` の指定については「[11.aunn コーディネート機能の実装について（Android）](./11.aunn%20コーディネート機能の実装について（Android）.md)」をご覧ください。
- 【Android】aunn コーディネート一覧の表示時間計測に対応しました。`AunnCoordinate.startCoordinateViewTimeTracking()` で、コーディネート一覧が画面内に 50% 以上・3 秒間表示されたときに計測を送信します。体型登録アプリの表示中は `pauseCoordinateViewTimeTracking()` / `resumeCoordinateViewTimeTracking()` で一時停止・再開できます。
- 【Android】`AunnCoordinateMeasurement.onDestroy()` で WebView を完全破棄するようにし、体型登録画面を閉じたあとのメモリリークを防止しました（破棄後のインスタンス再利用は不可）。
