...
## ドキュメントのバージョン情報
* 26/08/13 aunn コーディネートのエラーコード（500番台）を追加
* 24/06/07 初版
...
## aunn コーディネートのエラー
エラーメッセージは `AunnCoordinateListener` / `AunnCoordinateMeasurementListener` の `onFail(error: jp.co.makip.unisizesdk.UnisizeError)` で取得できます。
実装については [11.aunn コーディネート機能の実装について（Android）](./11.aunn%20コーディネート機能の実装について（Android）.md) を参照してください。

| エラーコード | エラーコード | メッセージ  | 詳細     |
| ------------ | ---------------- | -------- | -------- |
| 500          | AUNN_COORDINATE_API_ERROR | Error: Failed to fetch the aunn coordinate list.	 | aunn サーバーからのコーディネート一覧の取得に失敗	 |
| 501          | AUNN_ATTRIBUTE_API_ERROR | Error: Failed to fetch the user attribute from the UNISIZE API.	 | unisize サーバーからのユーザー属性の取得に失敗	 |
| 502          | AUNN_BEID_STATUS_API_ERROR | Error: Failed to fetch the beid status from the UNISIZE API.	 | unisize サーバーからの体型登録状態の取得に失敗	 |
| 503          | AUNN_TRACKING_API_ERROR | Error: Failed to send the aunn tracking log.	 | トラッキングログの送信に失敗	 |
| 504          | AUNN_PARAM_INSUFFICIENCY_ERROR | Error: An error occurred because a required aunn parameter was not set.	 | 必須パラメーター未設定。<br>※ SDK に設定したパラメーター関連のエラー	 |
| 510          | AUNN_MEASUREMENT_WEB_VIEW_JAVASCRIPT_ERROR | Error: A Javascript error has occurred in the measurement app.	 | 体型登録アプリの WebView で Javascript エラーが発生	 |
| 511          | AUNN_MEASUREMENT_WEB_VIEW_RECEIVED_ERROR | Error: An error occurred while loading the measurement app web resources.	 | 体型登録アプリの WebView ロード中にエラーが発生	 |
| 512          | AUNN_MEASUREMENT_WEB_VIEW_SSL_ERROR | Error: An SSL-related error has occurred while loading the measurement app.	 | 体型登録アプリの WebView で SSL エラーが発生	 |
